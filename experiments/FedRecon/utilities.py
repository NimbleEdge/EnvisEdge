...
DatasetSplitFn = Callable[[torch.Dataset, torch.tenosr], Tuple[torch.Datasest,
                                                                torch.Dataset]]
def build_dataset_split(recon_epochs_max: int = 1,
    ...
    recon_condition = lambda i, entry: torch.tensor(i)%torch.tensor(2)==0
    post_recon_condition = lambda i, entry: torch.tensor(i)%torch.tensor(2)> 0
