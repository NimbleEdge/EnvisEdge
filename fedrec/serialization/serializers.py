...
class JobSubmitMessageSerializer(AbstractSerializer):
    """
    This dumps the data into pkl format, if the file is specified the 
    workerstate data is pickled into the specified file, as workerstate might be 
    large. 
    The deserializer handles both the longer pkl string as well as the pkl dumped into the 
    file format.
    """
    @classmethod
    def serialize(cls, obj, file=None):
        assert type(obj) is JobSubmitMessage
        # JobSubmitMessage needs to be serialized before getting sent to the Kafka 
        # queue. 
        if file:
