...
class JobSubmitSerializer(AbstractSerializer):
    ...
    def serialize(self, obj):
        ...
        response_dict["job_args"] = [self.serialize_attribute(arg) for arg in obj.job_args]
        response_dict["job_kwargs"] = [self.serialize_attribute(kwarg) for kwarg in obj.job_kwargs]
    ...
    def deserialize(self, obj : Dict):
        job_args = [self.deserialize_attribute(arg) for arg in obj.job_args]
        job_kwargs = [self.deserialize_attribute(kwarg) for kwarg in obj.job_kwargs]
        worker_state = self.deserialize_attribute(obj.workerstate)

        JobSubmitMessage(obj["job_type"], job_args, job_kwargs, worker_state)
